...
```
The machine learning literature bristles with references to papers in statistics, computer science, artificial intelligence and a swathe of related scientific fields. A very crude citation analysis of the literature suggests that  the top 20 most cited publications in the field have focused on decision trees (Quinlan, Breiman), on support vector machines (Vapnik, Christiani, Cortes, Chang),  machine learning pedagogy (Mitchell, an early textbook; Witten, a textbook and software package on data mining; Duda, a textbook on pattern recognition) and somewhat lower, neural networks (Bishop).
...
> A classifier or classification rule is a function $d(\mathbf{x})$ defined on $\mathcal{X}$ so that for every $\mathbf{x}$, $d(\mathbf{x})$ is equal to one of the numbers $1, 2, ..., J$. [@breiman_cart_1984, 4]
 ...
 Second, classification is understood in terms of a series of binary splits that comprise the tree structure. The problem here is that many splits are possible. 
...
TBC -- the second form of movement -- to move the movement so that generalization becomes not just an outcome, but a mode of movement; 

```{r titanic, echo=TRUE, cache=TRUE, message=FALSE, warning=FALSE, comment=NA } 

    titanic = read.csv('data/titanic_train.csv')
    head(titanic)
    titanic_rp = rpart(Survived~Sex+Age+SibSp+Parch+Fare+Embarked, titanic)    
    summary(titanic_rp)
    plot(titanic_rp)
