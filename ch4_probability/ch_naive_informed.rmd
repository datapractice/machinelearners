...
The `bash` script carries out four different transformations of the data in building the model using command line tools such as `wc` (word count), `bc` (basic calculator), `grep` (text search using pattern matching) and `echo` (display a line of text). These tools or utilities are readily available in almost any UNIX-based operating system (e.g. Linux, MacOS, etc).  The first part of the code downloads the sample dataset of Enron emails (and I will discuss spam emails and their role in machine learning below). Note that this dataset has already been divided into two classes - 'spam' and 'ham' -- and individual emails of each class have been placed in separate directories or folders as text files. The second part of the code ('Part 2') counts the number of emails in each category and prints them. [HERE == describe the rest of this process -- the important points: it is only counting and multiplying things -- there are no other operations; it streams complicated objects into a series of counts; breaks things into a set of random variables with different probabilities] 
