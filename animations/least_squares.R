...
least_squares = function (x, y, n = 15, ani.type = c("slope", "intercept"), a, 
    b, a.range, b.range, ab.col = c("gray", "black"), est.pch = 19, 
    v.col = "red", v.lty = 2, rss.pch = 19, rss.type = "o", mfrow = c(1, 
        2), ...) 
{
    ...
    if (ani.type == "slope") {
        ...
        aseq = if (missing(a.range)) 
            seq(-5, 5, length = nmax)
        else seq(a.range[1], a.range[2], length = nmax)
        for (i in 1:nmax) {
            dev.hold()
            plot(x, y, ...)
            abline(fit, col = ab.col[1])
