...
virginica 2010 {[}1{]} 510216 3 discipline techniques year 1 statistics
time varying 2010 2 statistics sexpectation maximizationi 2010 3
statistics graphical model 2010 4 statistics markov random 2010 5
statistics structure learning 2010 6 statistics high dimensional 2010
{[}1{]} ``neural network'' ``clustering''\\ {[}3{]} ``k mean'' ``feature
selection''\\ {[}5{]} ``decision tree'' ``genetic algorithm''\\ {[}7{]}
``ensexpectation maximizationble'' ``pattern recognition''\\ {[}9{]}
``naive bayes'' ``random forest''\\{[}11{]} ``feature extraction''
``association rule''\\{[}13{]} ``sexpectation maximizationi'' ``time
serie''\\{[}15{]} ``maximum likelihood'' ``rough set''\\{[}17{]}
``algorithm'' ``knowledge discovery''\\{[}19{]} ``sexpectation
maximizationantic'' ``nearest neighbor''
...
machines), they do not appear in the statistics literature until in the
rankings. The prominence of neural nets on the engineering side of
