...
```{r iris_tree, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, comment=NA, size='smallsize', results='asis' } 
    data(iris)
    library(rpart)
    iris_tree =rpart(Species ~ ., iris)
```
...
```{r pattern_recognition1, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, comment=NA, size='smallsize', results='asis' } 
    query = 'select TI, PY, TC from basic_refs where TI like "%support vector machine%"'
    res = dbGetQuery(con, query)
    svm = res[order(res$TC, decreasing=TRUE)[1:20],]
    colnames(svm) = c('Title', 'Year', 'Citations')
    svm = svm[!duplicated(svm['Title']),]
    tab2 = xtable(svm,  label = 'tab:svm_lit',table.placement='ht',  align = c("p{0.05\\textwidth}",  "p{0.7\\textwidth}","p{0.1\\textwidth}",  "p{0.15\\textwidth}"), caption = 'Most cited papers on support vector machines')
