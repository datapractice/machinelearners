...
\chapter{Patterns of accumulation and dispersion}
\label{ch:pattern}
...
> The notion of pattern involves the concept of different modes of togetherness [@Whitehead_1956, 195-6]. \index{Whitehead, Alfred North!on pattern} \index{pattern!modes of togetherness} \index{Whitehead, Alfred North!on pattern}
...
```{r aid, echo=FALSE, results='asis', cache=TRUE } 
    ...
    tab  = xtable(res[1:10,], caption="References to Morgan and Sonquist's Automatic Interaction Detector", label='table:aid_citation', table.placement='ht')
...
\begin{lstlisting}[language=R,label={rpart_iris}]
...
```{r pattern_recognition1, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, comment=NA, size='smallsize', results='asis' } 
    ...
    tab2 = xtable(head(svm,20), label = 'tab:svm_lit',table.placement='ht',  align = c("p{0.05\\textwidth}",  "p{0.7\\textwidth}","p{0.1\\textwidth}",  "p{0.15\textwidth}"), caption = 'Most cited papers on support vector machines')
    print(tab2, type='latex', include.rownames=FALSE)
```
...
[^5.05]: The neural net, will be discussed in chapter \ref{ch:subject}.
